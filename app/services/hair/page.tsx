...
const BREADCRUMB = [{ href: '/', label: 'Home' }, { href: '/#services', label: 'Services' }, { label: 'Hair' }]
    ...
    <div className="bg-petal min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-16 relative z-10">
          ...
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div style={{ animation: 'fadeUp .8s ease both' }}>
              ...
              <Link href="/#contact"
                className="inline-block font-poppins text-sm font-semibold px-8 py-4 bg-rose text-white hover:bg-mauve transition-colors duration-300 rounded-full shadow-lg shadow-rose/20">
                Book Hair Session →
              </Link>
            </div>
            ...
            <div className="hidden md:block" style={{ animation: 'fadeUp .9s ease .15s both' }}>
              <div className="bg-white rounded-3xl p-8 shadow-xl shadow-rose/10 border border-blush/20">
                <p className="font-poppins text-xs tracking-widest uppercase text-rose font-semibold mb-6">Our Hair Categories</p>
                {CATEGORIES.map((c) => (
                  <div key={c.id} className="flex items-center gap-4 py-4 border-b border-blush/20 last:border-0">
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-16 py-28">
        <div className="text-center mb-20">
          <p className="font-poppins text-xs tracking-[0.3em] uppercase text-rose font-semibold mb-4">Full Menu</p>
          <h2 className="font-playfair text-5xl font-bold text-stone mb-4">What We <em>Offer</em></h2>
          <p className="font-poppins text-stone-light text-base max-w-xl mx-auto">Three comprehensive categories covering every hair need — from your daily style to a complete transformation.</p>
        </div>

        <div className="space-y-16">
          {CATEGORIES.map((cat, ci) => (
            <div key={cat.id} className="grid md:grid-cols-5 gap-8 items-start">
              {/* Category label */}
              <div className={`md:col-span-2 rounded-3xl p-8 h-full ${ci % 2 === 0 ? 'bg-cream' : ''}`}
                style={{ background: ci % 2 !== 0 ? cat.accent + '30' : undefined }}>
                ...
                <p className="font-poppins text-[10px] tracking-[0.3em] uppercase text-rose font-semibold mb-2">Category {cat.id}</p>
              ...
              <div className="md:col-span-3 grid sm:grid-cols-2 gap-4">
                {cat.services.map((s) => (
                  <div key={s.name} className="bg-white rounded-2xl p-5 border border-blush/20 hover:border-rose/30 hover:shadow-md hover:shadow-rose/10 transition-all duration-300 group">
                    ...
                    <p className="font-poppins text-xs text-stone-light leading-relaxed">{s.detail}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="bg-cream py-24">
        <div className="max-w-7xl mx-auto px-6 md:px-16">
          <div className="text-center mb-16">
            ...
            <p className="font-poppins text-stone-light text-sm">Four simple steps to salon-quality hair at home.</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={p.step} className="bg-white rounded-3xl p-7 border border-blush/20 relative">
                ...
                <p className="font-poppins text-xs text-stone-light leading-relaxed">{p.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          {FAQS.map((f) => (
            <div key={f.q} className="bg-white rounded-2xl p-6 border border-blush/20">
              ...
              <p className="font-poppins text-sm text-stone-light leading-relaxed">{f.a}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-6 md:px-16 pb-28">
        <div className="max-w-7xl mx-auto bg-stone rounded-[2.5rem] p-12 md:p-16 text-center relative overflow-hidden">
          ...
          <p className="font-poppins text-xs tracking-[0.3em] uppercase text-rose-light font-semibold mb-4 relative z-10">Ready to Glow?</p>
          <h2 className="font-playfair text-5xl font-bold text-white mb-6 relative z-10">Book Your Hair <em className="text-blush">Session Today</em></h2>
          <p className="font-poppins text-white/50 text-base max-w-lg mx-auto mb-10 relative z-10">Our certified hair artists are ready to come to you — fully equipped, on your schedule.</p>
          <div className="flex flex-wrap gap-4 justify-center relative z-10">
            <Link href="/#contact" className="font-poppins text-sm font-semibold px-8 py-4 bg-rose text-white hover:bg-mauve transition-colors duration-300 rounded-full">
              Book Now
            </Link>
            <Link href="/#services" className="font-poppins text-sm font-medium px-8 py-4 border border-white/20 text-white hover:bg-white/10 transition-colors duration-300 rounded-full">
              View All Services
            </Link>
          </div>
        </div>
