    ...
    <div className="bg-petal min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-16 py-24">
        <div className="text-center mb-14 reveal">
          <p className="font-poppins label-caps text-rose mb-3">Signature Experiences</p>
          ...
          <p className="font-poppins text-stone-light text-base max-w-xl mx-auto">
            Our most-loved signature facials — each a complete ritual designed for specific skin goals. Tap any card to explore the full experience.
          </p>
        </div>
          ...
          {([
            {
              ...
              desc: 'The signature Korean glass skin experience — deep double cleanse, enzyme exfoliation, sheet mask therapy, and lymphatic massage for that translucent, luminous glow.',
              accent: '#F0D8E8',
            },
          ] as const).map((item, i) => (
            <Link
              key={item.slug}
              href={item.slug}
              className={`reveal reveal-d${Math.min(i + 1, 6)} group relative bg-white rounded-2xl overflow-hidden border border-blush/20 hover:shadow-xl hover:shadow-rose/10 transition-all duration-300 flex flex-col`}
            ...
            >
              ...
              <div className="p-5 flex flex-col flex-1">
                <div className="flex items-center gap-3 mb-3">
                  <span className="font-playfair text-xl font-bold text-stone">{item.price}</span>
                  <span className="w-1 h-1 rounded-full bg-rose/40" />
                ...
                <p className="font-poppins text-sm text-stone-light leading-relaxed mb-4 flex-1">
                  {item.desc}
                </p>
